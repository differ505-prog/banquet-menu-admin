...
export const buildLibraryReviewPrompt = (library: RoleDishLibrary) => {
  ...
  return [
    ...
    "5. 對所有你認為命名不夠標準、不夠精確、過於口語、或不符合常見食譜書／料理資料庫寫法的名稱提出修正建議。這裡不只包含菜名，也包含像『左先鋒』『主帥』『靈魂洗滌』這類宴席位階／分類名稱；若你認為它們可以改成更接近食譜書、正式菜單分類、料理資料庫欄位的標準名稱，也可以提出對照建議。對菜系名稱也請一併審核，優先使用可見於食譜書或料理資料庫的標準分類；避免使用『融合菜』『創意菜』『私房菜』這類模糊標籤，若無法精準歸類，請標記『待人工確認』並說明原因。不要為了更好聽而任意升級食材、改變料理層級、或加入原本不存在的食材與修辭。",
