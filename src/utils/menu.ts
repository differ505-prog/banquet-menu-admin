...
export const buildLibraryReviewPrompt = (library: RoleDishLibrary) => {
  ...
  return [
    ...
    "5. 對所有你認為命名不夠標準、不夠精確、過於口語、或不符合常見食譜書／料理資料庫寫法的菜名，提出更標準、精確、可用於食譜書或正式菜單資料庫的名稱。不要為了更好聽而任意升級食材、改變料理層級、或加入原本不存在的食材與修辭。",
    "6. 請先彙整前面所有與『菜庫內容本身』有關的修改事項，另外整理成一段『可直接複製貼給 IDE 的菜庫修改提示詞』。這一段的目的，是讓 IDE 直接依據你的覆核結果去修改與擴增候選菜庫資料；必須明確列出要刪除、替換、改名、補充、增列的宴席地位、菜名、菜系、預製度與原因，並明確提醒 IDE 不需要拘泥於目前每個地位的候選數量。若涉及菜名修正，請以食譜書標準命名為優先，不要改寫成更華麗但偏離原菜色的名稱，也不要混入提示詞設計建議。",
    "7. 再另外整理一段『可直接複製貼給 IDE 的提示詞優化提示詞』，這一段主要用來優化未來給 LLM 的菜庫覆核提示詞；除了提示詞本身的結構與措辭，也可以納入系統設計面、資訊架構、欄位設計、輸入輸出格式、操作流程等會影響覆核品質的改善建議，但不要混入具體菜庫資料修改內容。這一段請強制使用「系統設計改善清單」格式輸出，至少包含：問題、影響、改善方向、建議做法、若要交給 IDE 執行時的提示詞草案。",
