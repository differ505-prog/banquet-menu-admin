...
export const buildLibraryReviewPrompt = (library: RoleDishLibrary) => {
  ...
  return [
    ...
    "2. 評估這份『給 LLM 的覆核提示詞』本身寫得好不好，給一個滿分 10 分的分數，並提出可讓 IDE 之後生成更高品質覆核提示詞的改善建議。這一段只討論提示詞設計本身，不要混入菜庫資料修改建議。請特別檢查它是否明確要求：食材多樣性防撞車、標準化欄位命名、以及 Context 情境輸入。",
    "3. 逐一檢查每個宴席地位是否放對類型的菜、菜系標示是否合理、預製度描述是否恰當；若有不適合該地位、菜系歸類可疑、命名不精確、或不利於宴席節奏的項目，請逐條指出。你必須額外執行『食材多樣性掃描』，檢查同一分類下的主要蛋白質、蔬菜類型、澱粉屬性是否重複過高，避免全豬肉、全白菜、全糯米等撞車現象。",
    "4. 針對每個宴席地位，提出菜色調整、修改或補充建議；若候選不足，請主動擴增菜品庫，補出更多適合同位階的候選菜。目標是把菜庫補強、補深、補廣，不是維持目前的候選數量；若有必要，可以明確建議某些地位增加到超過現有數量。補充建議時，請優先補進與現有食材屬性不同的候選。",
    "5. 對所有你認為命名不夠標準、不夠精確、過於口語、或不符合常見食譜書／料理資料庫寫法的名稱提出修正建議。這裡不只包含菜名，也包含像『左先鋒』『主帥』『靈魂洗滌』這類宴席位階／分類名稱；若你認為它們可以改成更接近食譜書、正式菜單分類、料理資料庫欄位的標準名稱，也可以提出對照建議。不要為了更好聽而任意升級食材、改變料理層級、或加入原本不存在的食材與修辭。",
    ...
    "7. 再另外整理一段『可直接複製貼給 IDE 的提示詞優化提示詞』，這一段主要用來優化未來給 LLM 的菜庫覆核提示詞；除了提示詞本身的結構與措辭，也可以納入系統設計面、資訊架構、欄位設計、輸入輸出格式、操作流程等會影響覆核品質的改善建議，但不要混入具體菜庫資料修改內容。這一段請強制使用「系統設計改善清單」格式輸出，至少包含：問題、影響、改善方向、建議做法、若要交給 IDE 執行時的提示詞草案，並把以下三項整併進新版 Prompt：A. 食材多樣性掃描；B. 標準化建檔 Schema；C. [Context] 區塊（至少含食用人數與設備限制）。",
