...
export const buildLibraryReviewPrompt = (library: RoleDishLibrary) => {
  ...
  return [
    ...
    "2. 評估這份『給 LLM 的覆核提示詞』本身寫得好不好，給一個滿分 10 分的分數，並提出可讓 IDE 之後生成更高品質提示詞的改善建議。",
    "3. 逐一檢查每個宴席地位是否放對類型的菜、菜系標示是否合理、預製度描述是否恰當；若有不適合該地位、菜系歸類可疑、命名不精確、或不利於宴席節奏的項目，請逐條指出。",
    "4. 針對每個宴席地位，提出菜色調整、修改或補充建議；若候選不足，也請補出適合同位階的候選菜，並盡量提供 1 至 3 道建議。",
    "5. 對所有你認為不夠正式、不夠精確、過於口語、或不利於宴席呈現的菜名，提出更正式、精確的完整稱呼。",
    "6. 最後請整理出一段『可直接複製貼給 IDE 的最終提示詞』，目的在於讓 IDE 之後能生成更好的菜庫覆核提示詞；這段最終提示詞必須完整、可直接複製使用，不要只給方向。",
    "7. 若某個宴席地位沒有問題，也要明確寫出『可保留』與原因，避免只挑錯不挑對。",
  ].join("\n");
};
