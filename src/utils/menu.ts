...
export const buildLibraryReviewPrompt = (library: RoleDishLibrary) => {
  ...
  return [
    ...
    "   - LLM 必須主動拆解菜名與常識食譜中的隱含食材，不可只依字面判讀主體食材。",
    "   - 若判斷菜色含有結球葉菜（如高麗菜、大白菜）或十字花科綠色葉菜（如青江菜、雪菜），無論其呈現方式為配菜、主體、切碎混入主食或藏在傳統做法中，皆須判定為違反高預製適配性並強制剔除，不可妥協。",
    "   - 對於菜飯、炒飯、米糕、湯底配料等高風險載體，必須額外檢查是否夾帶易出水、易黃化或覆熱後口感崩壞的葉菜成分。",
