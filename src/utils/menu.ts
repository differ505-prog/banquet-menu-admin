...
export const buildLibraryReviewPrompt = (library: RoleDishLibrary) => {
  ...
  return [
    ...
    "3. 逐一檢查每個宴席地位是否放對類型的菜、菜系標示是否合理、預製度描述是否恰當；若有不適合該地位、菜系歸類可疑、命名不精確、或不利於宴席節奏的項目，請逐條指出。你必須額外執行『食材多樣性掃描』『重複菜色／近似菜色掃描』與『高度預製適配性掃描』，檢查同一分類或跨分類是否出現主食材、烹法、味型過度相似的候選，避免全豬肉、全白菜、全糯米，以及同類型糖醋魚、排骨、雞湯重複堆疊；同時要主動標記那些不適合冷凍、解凍、覆熱、回蒸、回炸、微波的菜色，必要時建議直接剔除。若目前只是候選菜庫，請使用『候選同質性過高』『備選差異不足』『建議補入異質候選』等表述，避免直接寫成『撞車已發生』『不適合同時上桌』。",
